...
\section{Alternative Style for 1st Level
Header}\label{alternative-style-for-1st-level-header}

\subsection{Alternative Style for 1st Level
Header}\label{alternative-style-for-1st-level-header-1}

\emph{italic} (or \emph{italic}) \textbf{bold} (or \textbf{bold})
\texttt{code} superscript\textsuperscript{2} and
subscript\textsubscript{2} \texttt{Jay}
...
\item
  Item
\item
  sdf \#숫자하고 점만 입력하면 알아서 오내 개꿀이당
\item
  캡션을 달면 정상적이 아닌 경우 표시됨.
\end{enumerate}
...
\tightlist
\item
  자세한 사항은 Chat sheet markdownguide.org에 들어가면 많당.
\end{enumerate}
...
knitr::kable(mtcars{[}1:5, {]}, caption = ``Formatting table via
knitr::kable'')
